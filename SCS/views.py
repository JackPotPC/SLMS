...
from .utils import extract_cpu_idle, extract_memory_usage, extract_used_memory, ping_ip
...
def support_view(request):
    return render(request, 'SCS/support.html')
...
def list_servers_view(request):
    ...
    for server in servers:
        ping_result = ping_ip(server.server_connection_config.ip_address)
        server_ping_results.append(ping_result)
    combined_data = zip(servers, server_ping_results)
    return render(request, 'SCS/servers.html', {'servers': combined_data})
